...
- Migration of audio transcription from Gemini multimodal prompting to OpenAI's dedicated speech-to-text API
...
- Audio transcription uses a provider-specific service on purpose, without being incorrectly forced into the chat-model factory
...
9. Audio transcription is in scope, but it must use a dedicated transcription service path rather than the chat-model factory.
...
7. Audio transcription is migrated to OpenAI's dedicated transcription API without going through `getLlmModel(...)`.
8. Embedding model configuration remains outside the scope of this feature.
9. The repository can support the planned mapping of `gpt-5-mini` for `user-interaction` and `gpt-5-nano` for `helper` without requiring model-specific call-site changes.
...
3. Model-specific behavior differences for chat will be handled within the centralized model-selection layer or downstream testing, not by scattering provider logic through feature code.
4. Embeddings do not need to move as part of this feature’s first delivery slice.
5. Audio transcription can move to OpenAI independently of the chat-model factory.
...
- OpenAI transcription integration will have different request/response shapes from the current Gemini multimodal prompt path and must be validated separately.
